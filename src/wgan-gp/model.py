...
class OptionChainGenerator(keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            # __call__ objects
            z_mean, z_log_var, gen_features = self(X_input, training=True)
    ...
    def test_step(self, data):
        ...
        kl_loss, total_loss = self._compute_loss(
            z_mean, z_log_var, Y_real, gen_features, mode="val"
        )
    ...
    def _compute_loss(self, z_mean, log_var, y_real, generated_data, mode="Train"):
        ...
        kl_loss = -0.5 * tf.reduce_mean(
            tf.reduce_sum(
                1 + log_var_clipped - tf.square(z_mean) - tf.exp(log_var_clipped),
                axis=-1,
            )
        )
...
def create_adjacency_matrix(options, threshold=5):
    ...
    for i in range(num_options):
        for j in range(i, num_options):
            ...
            if (
                abs(options[i]["strike_price"] - options[j]["strike_price"])
                <= threshold
                and options[i]["dte"] == options[j]["dte"]
            ):
